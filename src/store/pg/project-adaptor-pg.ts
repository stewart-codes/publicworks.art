...
const mapWorkPgToEntity = (w: WorkPg): WorkEntity => {
  ...
  out.dutchAuctionDeclinePeriodSeconds =
    w.dutchAuctionDeclinePeriodSeconds ?? null;
  out.dutchAuctionDecayRate =
    w.dutchAuctionDecayRate != null
      ? parseFloat(w.dutchAuctionDecayRate)
      : null;
...
export class RepositoryPgAdaptor implements ProjectRepositoryI {
  ...
  async getAllTokensWithStatus(
    ...
    const rows = await this.repo.getTokensByStatus(
      chainInfo().chainId,
      status,
      limit
    );
  ...
  async getProjectTokensWithStatus(
    ...
    const rows = await this.repo.getTokensByWorkAndStatus(
      parseInt(projectId),
      status,
      limit
    );
  ...
  async getProjectAndTokenById(
    ...
    const work = await this.repo.getWorkById(
      chainInfo().chainId,
      parseInt(projectId)
    );
    if (!work) return null;
    const token = await this.repo.getTokenBySg721AndTokenId(
      work.sg721 ?? "",
      parseInt(tokenId)
    );
    if (!token) return null;
    return {
      project: mapWorkPgToEntity(work),
      token: mapTokenPgToEntity(token),
    };
  }
  ...
  async getProjectPreviewImage(id: string): Promise<TokenEntity | null> {
    ...
    if (!work || !work.sg721) return null;
    const { items } = await this.repo.getTokensByWork(parseInt(id), {
      limit: 1,
    });
  ...
  async getProjectTokens2({
    ...
    const afterTokenId =
      offset != null ? parseInt(offset.toString()) : undefined;
    const { items, nextTokenId } = await this.repo.getTokensByWork(workId, {
      limit,
      afterTokenId,
    });
    return {
  ...
  async getProjects({
    ...
    if (typeof offset === "number")
      throw new Error(
        "numeric offset not supported for postgres; pass a string cursor"
      );
  ...
  async updateProject(
    ...
    try {
      ...
      if (request.externalLink !== undefined)
        updates.externalLink = request.externalLink ?? null;
      if (request.startDate !== undefined)
        updates.startDate = new Date(request.startDate);
      ...
      if (request.pixelRatio !== undefined)
        updates.pixelRatio = request.pixelRatio;
      if (request.priceStars !== undefined)
        updates.priceStars = request.priceStars;
      if (request.maxTokens !== undefined)
        updates.maxTokens = request.maxTokens;
      if (request.royaltyPercent !== undefined)
        updates.royaltyPercent = request.royaltyPercent;
      ...
      if (request.codeCid !== undefined)
        updates.codeCid = request.codeCid ?? null;
      if (request.coverImageCid !== undefined)
        updates.coverImageCid = request.coverImageCid ?? null;
      if (request.license !== undefined)
        updates.license = request.license ?? null;
      if (request.sg721CodeId !== undefined)
        updates.sg721CodeId = request.sg721CodeId ?? null;
      if (request.minterCodeId !== undefined)
        updates.minterCodeId = request.minterCodeId ?? null;
      if (request.isDutchAuction !== undefined)
        updates.isDutchAuction = request.isDutchAuction;
      if (request.dutchAuctionEndPrice !== undefined)
        updates.dutchAuctionEndPrice =
          request.dutchAuctionEndPrice != null
            ? request.dutchAuctionEndPrice.toString()
            : null;
      ...
      if (request.dutchAuctionDeclinePeriodSeconds !== undefined)
        updates.dutchAuctionDeclinePeriodSeconds =
          request.dutchAuctionDeclinePeriodSeconds ?? null;
      if (request.dutchAuctionDecayRate !== undefined)
        updates.dutchAuctionDecayRate =
          request.dutchAuctionDecayRate != null
            ? request.dutchAuctionDecayRate.toString()
            : null;
