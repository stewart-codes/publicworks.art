...
export class RepositoryPg {
  ...
  constructor(private readonly ds: DataSource) {
    this.initializedPromise = ds.isInitialized
      ? Promise.resolve()
      : (ds.initialize() as unknown as Promise<void>);
  }
  ...
  async findPublishedWorks({
    ...
    items: WorkPg[];
    nextCursor: { startDate: string; id: number } | undefined;
  }> {
    await this.initializedPromise;
    ...
    if (!includeHidden)
      qb = qb.andWhere("w.hidden = :hidden", { hidden: false });
    ...
    if (cursor) {
      if (order === "desc") {
        qb = qb.andWhere(
          "(w.startDate < :sd OR (w.startDate = :sd AND w.id < :cid))",
          {
    ...
    return {
      items,
      nextCursor: hasMore
        ? { startDate: last.startDate.toISOString(), id: last.id }
        : undefined,
    };
  }
  ...
  async getTokenBySg721AndTokenId(
    sg721: string,
    tokenId: number
  ): Promise<WorkTokenPg | null> {
    await this.initializedPromise;
  ...
  async getTokensByWork(
    ...
    return {
      items,
      nextTokenId: hasMore ? items[items.length - 1].tokenId : undefined,
    };
  }

  // --- Block Heights ---

  async upsertBlockHeight(
    chainId: string,
    name: string,
    height: bigint
  ): Promise<void> {
  ...
  async createUser(
    input: Omit<UserPg, "createdAt" | "updatedAt">
  ): Promise<UserPg> {
    await this.initializedPromise;
